...
echo WILL BE `sqlite3 test.db "select case when value==0 then 'CLOSING' when value==1 then 'OPENING' else 'UNKNOWN' end, datetime(date, 'unixepoch', 'localtime') from requests where textstate=='scheduled' order by date asc limit 1;"`
